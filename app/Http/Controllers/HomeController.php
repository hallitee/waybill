...
class HomeController extends Controller
{
    ...
    public function index()
    {
		...
		$q->where('deliveredTo', $name)->orWhere('sentFrom', $loc)->orWhere('sentTo', $loc);})->where('receiveStatus', 'OPEN')->where('wType', 'TRANSFER')->count();
		//$tPend = doc::where(function($q) use ($loc, $name){
		...
		$q->where('deliveredTo', $name)->orWhere('sentTo', $loc)->orWhere('sentFrom', $loc)->orWhere('sentBy', $name);})->where('receiveStatus', 'OPEN')->where('wType', 'LOAN')->count();
		...
		$q->where('deliveredTo', $name)->orWhere('sentTo', $loc)->orWhere('sentFrom', $loc)->orWhere('sentBy', $name);})->where('receiveStatus', 'OPEN')->where('wType', 'REPAIR')->count();		//Repair 
